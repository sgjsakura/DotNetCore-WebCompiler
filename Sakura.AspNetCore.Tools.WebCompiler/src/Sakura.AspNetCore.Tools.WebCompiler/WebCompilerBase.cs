...
namespace Sakura.AspNetCore.Tools.WebCompiler
{
		...
		where TOptions : WebCompilerWorkItemOptions
	{
		...
		{
			get { throw new NotImplementedException("The derived class must implement this property."); }
		}
		...
		/// <param name="workItem">The work item.</param>
		/// <returns>A collection contains all output in the work item.</returns>
		IEnumerable<string> IWebCompiler.GetDefaultOutputFile(WebCompilerWorkItem workItem)
			=> GetDefaultOutputFile(ConvertWorkItem(workItem));
		...
		protected virtual WebCompilerResult CompileContent(string content, string inputFileName, string outputFileName,
			TOptions options)
		{
		...
		public virtual string GetSourceMapPath(string sourcePath, string outputPath, TOptions options)
			=> GetOutputPath(sourcePath, outputPath, options) + ".map";
	}
}